...
from convs.modules import get_child_dict
...
class DERNet(nn.Module):
    ...
    def forward(self, x, params=None, activation='relu'):
        outputs = [self.convnets[i](x, get_child_dict(params, 'convnets.{}'.format(i)), activation) for i in range(len(self.convnets))]
        ...
        out.update({"aux_logits": aux_logits, "features": features, "fmaps": fmaps})
        '''
        {
            'features': features
            'logits': logits
